...
{
	RECORD* rec = &robot->game.rec;
	//HISTORY_HAND (* history)[180] =  rec->history;
	COMBO_OF_POKERS* h;
	int i;
	if(!rec)
	{
		g_pLogger->Log("error in function(print_history_content),rec is null.");
		return;
	}
	g_pLogger->Log("lord:%d,winner:%d,bombed:%d,now:%d",rec->lord,rec->winner,rec->bombed,rec->now);
	for (i=0; i<rec->now; i++)
	{
		h = &rec->history[i].h;
		if(h)
			g_pLogger->Log("player:%d, type:%d,low:%d,len:%d,control:%d,three_desc:%d/%d/%d/%d/%d",
				(PLAYER_POS)rec->history[i].player,
				h->type,
				h->low,
				h->len,
				h->control,
...
void print_hand_poker2(POKERS * h )
{
	int k = 0,i=0;
	char hand[max_POKER_NUM+1];
	for (i=0; i<MAX_POKER_KIND; i++)
	{
		for (int j=0 ; j<h->hands[i]; j++){
			hand[k++] = poker_to_char(i);
		}
	}
	hand[k] = '\0';
	g_pLogger->Log("%s",hand);
}

void print_combo_poker2(COMBO_OF_POKERS * h /*, COMBO_OF_POKERS *h1 /*for three*/  )
{
	int k = 0;
	char hand[max_POKER_NUM+1];
	char c;
	...
	case THREE_SERIES:
		...
		break;
	case BOMB:
		c = poker_to_char(h->low);
		g_pLogger->Log("%c%c%c%c",c,c,c,c);
		break;
	case THREE:
		c = poker_to_char(h->low);
		g_pLogger->Log("%c%c%c",c,c,c);
		break;
	case PAIRS:
		c = poker_to_char(h->low);
		g_pLogger->Log("%c%c",c,c);
		break;
	case SINGLE:
		g_pLogger->Log("%c",poker_to_char(h->low));
		break;
	case 31: //31
...
void print_game_record(LordRobot *robot,void (*pLog)(const char* fmt, ...))
{
	...
	g_pLogger->Log("=========GAME RECORD=======");

	h =  &rec->p[CUR_PLAYER];
	g_pLogger->Log("===CUR PLAYER===total %d ",h->total);
	print_hand_poker2(h);
	h =  &rec->p[DOWN_PLAYER];
	g_pLogger->Log("===DOWN PLAYER===total %d ",h->total);
	print_hand_poker2(h);
	h =  &rec->p[UP_PLAYER];
	g_pLogger->Log("===UP PLAYER===total %d ",h->total);
	print_hand_poker2(h);
	h =  &rec->pot;
	g_pLogger->Log("=== POT ===total %d ",h->total);
	print_hand_poker2(h);
	g_pLogger->Log("===   LORD is  %s    ===", rec->lord==CUR_PLAYER? "CUR_PLAYER" :
		(rec->lord==DOWN_PLAYER? "DOWN_PLAYER" : "UP_PLAYER") );
	g_pLogger->Log("===   CUR_PLAYER is  %s    ===",TYPE[ rec->type[CUR_PLAYER]]);
	/*
	...
	for(int i=0; i<rec->now; i++)
	{
		...
		g_pLogger->Log("[%s] " , TYPE[rec->type[pos]]);
...
void print_history_content2(LordRobot *robot, void (*pLog)(const char* fmt, ...))
{
	...
	for(int i=0; i<rec->now; i++)
	{
		PLAYER_POS pos= rec->history[i].player;
		g_pLogger->Log("[%s] " , TYPE[rec->type[pos]]);
		print_combo_poker2(&rec->history[i].h);
...
COMBO_OF_POKERS*  find_smallest_in_combos(COMBO_OF_POKERS* com, int total,PLAYER* player ,BOOL not_search_single)
{
	...
	if ( !not_search_single) {
		...
		{   //max len..
			...
			if(cur==NULL)
			{
				printf("s->biggest_num %d\n",s->biggest_num  );
				printf("s->not_biggest_num %d\n",s->not_biggest_num  );	
				printf("player->h->total %d\n",player->h->total  );
				printf("s->real_total_num %d\n",s->real_total_num  );                   	
			}
			max= s->biggest[0]->low;
...
{ //参数  第一还是游戏控制，第二个是传入玩家牌的结构体，第三个是玩家的组合叙述结构体， 组合的摘要，当前玩家。
	int combo_nums=0;
	/*
	//serach four
...
Get the
*/
int search_combos_in_suit(GAME* game, POKERS *h , POKERS *opp, PLAYER* player ) //  传入的参数分别是游戏控制着 ，玩家游戏牌类，剩余的外面的牌，还有当前的玩家
{
...
game_turn_p1:
		...
		{
			robot[0] = NULL;
			//SERVER_LOG("robot[0]----------------------------%d", robot[0]->game.players[2]->h->total);
			break;
		}
...
void generate_perhaps_nodes(CNODE * t)
{
	...
	if(first)
	{
		...
		DBG(print_all_combos(combos, num));
...
int get_node_value(CNODE * t)
{
	...
	PRINTF(LEVEL,"\n");
	DBG(print_hand_poker(&t->h->p));
	DBG(print_hand_poker(&t->h1->p));
	DBG(print_hand_poker(&t->h2->p));
	//*/
	if(t->c.type!=NOTHING) {
...
int get_the_best_hand_2(GAME *game,  COMBO_OF_POKERS * cur,bool first)
{
	...
	*cur = top.childs[max_value_item].c;
	...
	else
	{
		if(first ==1) 
		{
			PRINTF_ALWAYS("line %d, fatal error, must play some cards..\n", __LINE__);
		}
		return 1;
	}
}
...
int  setRobotLevel(LordRobot* robot,int level)
{
	...
	game->known_others_poker = level>=1?1:0;	
	game->use_best_for_undefined_case = level>=2?1:0;	
	return level;
}
...
void initCard(LordRobot* robot, int* robot_card, int* up_player_card, int* down_player_card)
{
	...
	//  if (!set->cus) //rand
	{
		...
		{
			if(robot_card[i]<0||robot_card[i]>53) break;
			...
			if(up_player_card[i]<52)
				...
				PRINTF(LEVEL,"SUCks\n");
			if (down_player_card[i]>=0 && down_player_card[i]<=53)
			{
	...
	DBG(print_hand_poker(game->players[2]->h));
	DBG(print_card(game->players[2]->card));
	...
	{
		full_poker(&all);
		sub_poker(&all, game->players[DOWN_PLAYER]->h, &all); //去除二号玩家的牌才是真正的总共的牌
		sub_poker(&game->all, game->players[DOWN_PLAYER]->h, &game->all);
		for (int j = 0; j < 3; j++)
		{
			game->suits[j].h = &game->suits[j].p;
			game->suits[j].opps = &game->suits[j].opp;
			...
			{
				game->suits[j].oppDown_num = 17;
				game->suits[j].oppUp_num = 4;
			}
			else if (j == CUR_PLAYER)
			{
				game->suits[j].oppDown_num = 4;
				game->suits[j].oppUp_num = 17;
			}

		}

		for (int i = 0; i < 21;i++)//   把二号玩家的牌直接重置为厨师状态
		{
			down_player_card[i] = -1;
			game->players[DOWN_PLAYER]->card[i] = -1;
		}
		down_player_card[0] = 0;
		down_player_card[1] = 1;
		down_player_card[2] = 13;
		down_player_card[3] = 14;
		game->players[DOWN_PLAYER]->card[0] = 0;
		game->players[DOWN_PLAYER]->card[1] = 1;
		game->players[DOWN_PLAYER]->card[2] = 13;
		game->players[DOWN_PLAYER]->card[3] = 14;

		for (int i = 0; i < 15; i++) //把下家的手牌给重置掉
		{
			game->players[DOWN_PLAYER]->h->hands[i] = 0;
		}
		game->players[DOWN_PLAYER]->h->hands[0] = 2;
		game->players[DOWN_PLAYER]->h->hands[1] = 2;
		add_poker(&all, game->players[DOWN_PLAYER]->h, &all);
		add_poker(&game->all, game->players[DOWN_PLAYER]->h, &game->all);
		sub_poker(&all, game->players[CUR_PLAYER]->h, game->players[CUR_PLAYER]->opps);//如果有两个人那么要把下家的牌给排除掉
		sort_poker(game->players[CUR_PLAYER]->opps);
		sub_poker(&all, game->players[UP_PLAYER]->h, game->players[UP_PLAYER]->opps);
		sort_poker(game->players[UP_PLAYER]->opps);
		...
		sort_poker(game->players[1]->h);
		sort_poker(game->players[0]->h);
		SERVER_LOG("----------------initCard2------- game->pot->total-------------------------%d", game->pot->total);
		game->players[0]->cur = NULL;
		game->players[1]->cur = NULL;
		//    log_brief =0;
		...
		game->rec.p[DOWN_PLAYER] = *game->players[DOWN_PLAYER]->h;
		game->rec.p[UP_PLAYER] = *game->players[UP_PLAYER]->h;
		SERVER_LOG("----------------initCard3------- game->pot->total-------------------------%d", game->pot->total);
	}
	return ;
#endif
}
...

	{
		...
		for(int k=0; k<54; k++)
		{
			if(card_used[k]==0)
			{
				...
				PRINTF_ALWAYS("line %d: FATAL ERROR in check_all_card_is_ok:%d\n",__LINE__,k+3);	 	
				return k+3;
			}
		}
	}
...
void setLaizi(LordRobot* robot, int laizi)
{
	...
	game->hun = laizi;		
	game->all.hun = laizi;
    game->rec.winner = laizi;
}

int get_laizi_from_pot(LordRobot* robot)
{
	POKERS * h = robot->game.pot;
	int laizi = 0;
	if(h->hands[h->begin] >= 2){
		laizi=h->begin;
	}else{		
...
int callScore(LordRobot* robot)
{
	...
	if(game->known_others_poker)
	{
		...
		if(game->hun != -1)
		{
			//following codes just used by laizi
			//int laizi=get_laizi_from_pot(&tR);
			//setLaizi(&tR, laizi);
			...
			search_combos_in_suit(&tR.game, uppl ->h,uppl ->opps,dnpl );
			search_combos_in_suit(&tR.game, dnpl ->h,dnpl ->opps,dnpl );
			
			int opp_bomb= get_opp_bomb_number(&tR.game); 
			int own_bomb = get_bomb_numbers_hun(pl->h);

			if( (pl->summary->combo_total_num <= 1+pl->summary->bomb_num)
				|| (pl->summary->combo_total_num <=3 && pl->summary->bomb_num>=opp_bomb)
				|| (HUN_NUMBER(pl->h) >=3)
				)
			{
				winner = 1;
			}
			else if((CONTROL_SUB_COMBO_NUM(pl)>-20 && (pl->summary->bomb_num>0 || !opp_hasbomb(&tR.game)))
				|| (CONTROL_SUB_COMBO_NUM(pl)>-10)
				|| (  (CONTROL_SUB_COMBO_NUM(pl) >= (CONTROL_SUB_COMBO_NUM(dnpl)+1) )
				     && (CONTROL_SUB_COMBO_NUM(pl) >= (CONTROL_SUB_COMBO_NUM(uppl)+1)  ))
				  || (own_bomb > opp_bomb)
				  || (pl->summary->bomb_num >= opp_bomb)
				)
			{
				winner=1;
			}
		...
		PLAYER * pl= tR.game.players[CUR_PLAYER];
			search_combos_in_suit(&tR.game, pl ->h,pl ->opps,pl );
 	        if((CONTROL_SUB_COMBO_NUM(pl)>=-30 )
			   || (CONTROL_NUM(pl)>=20)
			   )
			winner =1;			
		   }
	...
	int bomb_numb = game->hun==-1? get_bomb_numbers(game->players[2]->h)
		                         : get_bomb_numbers_hun(game->players[2]->h) ;
	int score = ((control+bomb_numb*10)>=10)?3:-1;
	game->rec.score[(int)CUR_PLAYER]=score;
	game->call.player[game->call.cur_num]=CUR_PLAYER;
	game->call.score[game->call.cur_num]=score;
	game->call.cur_num++;
...
void beLord(LordRobot* robot,int* robot_card,int player_number)
{
	...
	if(player_number == 2)
	{
		...
		{
			add_poker(game->players[DOWN_PLAYER]->h, game->players[UP_PLAYER]->h, game->players[CUR_PLAYER]->opps);
			sort_poker(game->players[CUR_PLAYER]->opps);
		}
...
void getPlayerTakeOutCards(LordRobot* robot,int* poker, int * laizi, int player_number)
{
	...
	if (poker[0]!=-1)
	{
		...
		if(game->rec.now>0)
		{
			...
			{
				printf("strange1 in getPlayerTakeOutCards!\n");
				print_history_content(robot,NULL);
				// while(1);
			}
		}
	...
	else
	{
		...
		if(game->rec.now>0)
		{
			if( player_number!=((game->rec.history[game->rec.now-1].player +1)%3))
			{
				printf("strange2 in getPlayerTakeOutCards!\n");
				print_history_content(robot,NULL);
				// while(1);
			}
		}
...
int takeOut(LordRobot* robot,int * p, int* laizi)
{
	...
	else
	{
		...
		if(first)
		{
			PRINTF_ALWAYS("NO card!just return a 3\n");
			cur->type=SINGLE;
			cur->low=P3;
	...
	if(!pass) {
		...
		if(game->rec.now>0)
		{
			if( UP_PLAYER!=game->rec.history[game->rec.now-1].player )
			{
				printf("strange in takeout!\n");
				print_history_content(robot,NULL);
				// while(1);
			}
		}
...
int  arrange_poker_default(LordRobot * robot[3],int *pot, int laizi)
{
	for (int k=0; k<54; k++)
	{
		all_cards[k]=1;
...
int  arrange_poker(LordRobot * robot[3],int *pot, int laizi)
{
	...
	if(fp == NULL)
	{
		laizi = arrange_poker_default(robot, pot, laizi);		
	}else{
...
int main(int argc, char** argv)
{
	...
	test_set.computer[1]=0;
	test_set.computer[2]=0;
...
NEW_GAME:
	...
	printf("player 0 is %s\n",test_set.computer[0]?"COM":"HUMAN");
	printf("player 1 is %s\n",test_set.computer[1]?"COM":"HUMAN");
	printf("player 2 is %s\n\n\n",test_set.computer[2]?"COM":"HUMAN");
	...
	else if (end==8)
	{
		test_set.cus=2;
		if(argc>1)
			strncpy(test_set.filename,argv[1],999);
		else
		{
			printf("input file:");
			scanf("%s", test_set.filename);
		}
		if ( (test_set.input= fopen(test_set.filename,"rb"))==NULL)
		{
			printf("file could not be open!\n");
			goto NEW_GAME;
		}
		sprintf(filename, "result_%s", test_set.filename);
		if ( (logfile= fopen(filename,"wb"))==NULL)
		{
			printf("could not open log %s", filename);
			// exit(0);
		}
		sprintf(filename, "brief_result_%s", test_set.filename);
...
INIT_GAME:
	...
	if (test_set.cus==2)
	{
		init_cus_game(&robot[0]->game,&test_set);
		lord = robot[0]->game.cur_playernum;

		DUMP_GAME_FOR_PLAYER(&robot[0]->game);
	...
	else
	{
		...
		if(end==9) 	{
			...
			fclose(test_set.input);
		}
		else
		{
			Request res ={4,60,60,0,70};
			if(laizi!=-1)
				laizi=rand()%13;
			printf("laizi is %d\n",laizi);
