...
pt_gc_rob_lord_req_handler::pt_gc_rob_lord_req_handler(void)
{
}
...
void pt_gc_rob_lord_req_handler::handler( const pt_gc_rob_lord_req& req,CUserSession* session )
{
	SERVER_LOG("pt_gc_rob_lord_req_handler:lord = %d, guid:%lld",req.cDefaultLord, session->ply_guid());
	...
	session->set_status(CUserSession::US_ROB);
}
